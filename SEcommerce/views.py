...
def registration(request):
    if request.method == "POST":
        ...
        myuser.save()
        if username is not None and password is not None and fullname is not None and address is not None and contactno is not None and confirm is not None and confirm == password:
            messages.success(request, "Your data has been saved successfully.")
        elif username is not None and password is not None and fullname is not None and address is not None and contactno is not None and confirm is not None and confirm != password:
            messages.error(request, "Passwords do not match.")
            return render(request, "SECHTML/custreg.html")
        else:
            messages.error(request, "All fields must be filled.")
        return redirect("registration")
    return render(request, "SECHTML/custreg.html")

def editproflandingpage(request):
    return render(request, "SECHTML/editproflandingpage.html")
